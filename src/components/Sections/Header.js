...
const NavItem = memo(({section, current, inactiveClass, activeClass, onClick}) => {
  return (
      <a href={`/#${section}`} passHref className={classNames(current ? activeClass : inactiveClass)} key={section} onClick={onClick}>
        {section}
      </a>
  );
});
